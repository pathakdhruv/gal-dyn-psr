...
def err_DT91(bdeg, sigb, ldeg, sigl, dkpc, sigd):
    ...
    def VpbyRs():
        a = (Vsms*b0dt91*Rp/(Rs*Rs))-((Vsms/Rs)*b0dt91*RpbyRs())
        return a;
    ...
    err_plsq = pow(diffbyb(),2.0)*pow(sigb,2.0) + pow(diffbyl(),2.0)*pow(sigl,2.0) + pow(diffbyd(),2.0)*pow(sigd,2.0) + pow(diffbyVs(),2.0)*pow(sigVs,2.0) + pow(diffbyRs(),2.0)*pow(sigRs,2.0) + pow(diffbyb0dt(),2.0)*pow(sigb0dt,2.0)
